<template>
  <div class="llm-config-management">
    ...
    <!-- 创建/编辑配置对话框：表单区固定高度可滚动，底部按钮始终可见 -->
    <el-dialog
      ...
      @closed="resetForm"
    >
      <div class="llm-config-dialog-scroll">
      <el-form
        ref="formRef"
        :model="configForm"
        :rules="formRules"
      ...
      >
        ...
        <el-form-item label="模型名称" prop="model_name">
          <el-input
            ...
            placeholder="请输入模型名称，如：doubao-pro-32k"
          />
        ...
        <el-form-item label="API 地址" prop="base_url">
          ...
          <el-text size="small" type="info" style="display: block; margin-top: 5px;">
            常见配置：OpenAI: https://api.openai.com/v1 | DeepSeek: https://api.deepseek.com | 通义千问: https://dashscope.aliyuncs.com/compatible-mode/v1 | 豆包: https://ark.cn-beijing.volces.com/api/v3
          </el-text>
        ...
        <el-form-item label="上下文限制" prop="context_limit">
          ...
          <el-text size="small" type="info">
            模型最大上下文Token数（GPT-4o: 128000, Claude: 200000, Gemini: 1000000, 豆包: 32000/128000）
          </el-text>
        </el-form-item>
        ...
        <el-form-item label="多模态支持">
          ...
          />
          <el-text size="small" type="info" style="display: block; margin-top: 5px;">
            标识模型是否支持图片/多模态输入（如GPT-4V、Qwen-VL、豆包Vision等）
          </el-text>
        </el-form-item>
...
const availableModels = computed(() => {
  const modelMap: Record<string, string[]> = {
    ...
    doubao: ['doubao-pro-32k', 'doubao-pro-128k', 'doubao-lite-32k', 'doubao-lite-128k'],
    other: []
  }
...
const handleProviderChange = () => {
  ...
  // 根据提供商设置默认的上下文限制
  const defaultContextLimits: Record<string, number> = {
    openai: 128000,
    claude: 200000,
    gemini: 1000000,
    qwen: 128000,
  ...
  configForm.context_limit = defaultContextLimits[configForm.provider] || 128000
}
...
.llm-config-dialog-scroll::-webkit-scrollbar {
  width: 6px;
}

.llm-config-dialog-scroll::-webkit-scrollbar-thumb {
  background: var(--el-border-color-darker);
  border-radius: 3px;
}
</style>

<!-- Dialog 挂载到 body，不用 scoped 才能稳定作用在 .el-dialog 上 -->
<style>
...
.llm-config-dialog .el-dialog__body {
  flex: 1;
  min-height: 0;
  overflow: hidden;
  padding-top: 8px;
