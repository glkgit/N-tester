...
# 导入真实的 LLM 生成器
from .ai_generator_real import RealAITestCaseGenerator
            ...
            try:
                if source_type == "document":
                    ...
                    try:
                        ...
                        content = doc_data.get("content", document.content or "")
                        images = doc_data.get("images", [])
                    except Exception as e:
                        logger.warning(f"使用文档处理器提取内容失败: {e}")
                    ...
                    if images:
                        image_info = "\n\n【文档中的图片信息】"
                        for img in images:
                            image_info += f"\n- {img.get('description', '图片')}: {img.get('note', '')}"
                        image_info += "\n\n请注意：文档中包含图片，请根据图片描述来理解需求并生成测试用例。如果图片是UI截图或流程图，请结合图片内容来理解功能需求。"
                        enhanced_context += image_info
                        logger.info(f"文档包含 {len(images)} 张图片，已添加到上下文中")
...
async def _generate_testcase_mock(
    ...
    try:
        ...
        logger.info(f"[_generate_testcase_mock] 使用真实的 LLM 生成器: provider={llm_config.provider}, model={llm_config.model_name}")
        ...
        if count == 1:
            ...
            testcases_data = await generator.generate_multiple_testcases(
                enhanced_requirement, count, enhanced_context
            )
...
async def optimize_existing_testcase(
    ...
    try:
        ...
        logger.info(f"[optimize_existing_testcase] 使用真实的 LLM 生成器: provider={llm_config.provider}, model={llm_config.model_name}")
...
async def get_requirement_sources(request: Request, project_id: int):
    ...
    try:
        ...
        # 暂时跳过权限检查，避免认证问题
        # TODO: 后续需要完善认证机制
        # if not await ProjectMember.filter(
        #     project=project, user_id=request.state.user.id
        # ).exists():
        #     return request.app.forbidden(msg="无权限访问此项目")
...
async def generate_testcase_conversation_api(
    ...
    try:
        ...
        # 获取生成数量，默认为3，不设上限
        count = conversation_request.get("count", 3)
        ...
        logger.info(f"[generate_testcase_conversation_api] 使用真实的 LLM 生成器: provider={llm_config.provider}, model={llm_config.model_name}")
