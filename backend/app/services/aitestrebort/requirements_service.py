...
class DocumentProcessor:
    ...
    def extract_content_with_images(self, document: RequirementDocument) -> dict:
        """
        提取文档内容，包含图片信息
        返回: {"content": str, "images": list[dict]}
        """
        ...
        try:
            if document.file_path and os.path.exists(document.file_path):
                file_extension = document.file_path.lower().split('.')[-1] if '.' in document.file_path else ''

                if file_extension == 'pdf':
                    images = self._extract_images_from_pdf(document.file_path)
                elif file_extension in ['docx', 'doc']:
                    images = self._extract_images_from_word(document.file_path)
                elif file_extension == 'md':
                    images = self._extract_images_from_markdown(document.file_path)

                logger.info(f"从文档中提取了 {len(images)} 张图片")

        except Exception as e:
            logger.warning(f"提取文档图片失败: {e}")
                        ...
                        if obj.get('/Subtype') == '/Image':
                            try:
                                ...
                                if '/Filter' in obj:
                                    ...
                                    if 'DCT' in filter_type or 'JFIF' in filter_type:
                                        # JPEG格式
                                        img_data = obj.get_data()
                                        img_base64 = base64.b64encode(img_data).decode('utf-8')
                                        mime_type = 'image/jpeg'
                                    elif 'Flate' in filter_type:
                                        # PNG格式
                                        img_data = obj.get_data()
                                        img_base64 = base64.b64encode(img_data).decode('utf-8')
                                        mime_type = 'image/png'
                                ...
                                images.append({
                                    ...
                                    "page": page_num,
                                    "index": len(images) + 1,
                                    "description": f"PDF第{page_num}页的图片",
                                    "base64": f"data:{mime_type};base64,{img_base64}",
                                    "mime_type": mime_type
                                })
                            except Exception as img_err:
                                logger.warning(f"提取PDF图片失败: {img_err}")
                                continue
        except Exception as e:
            logger.warning(f"PDF图片提取失败: {e}")
        return images

    def _extract_images_from_word(self, file_path: str) -> list:
                ...
                if "image" in rel.target_ref:
                    try:
                        ...
                        images.append({
                            "type": "word_image",
                            "index": image_index,
                            "description": f"Word文档第{image_index}张图片",
                            "base64": f"data:{content_type};base64,{img_base64}",
                            "mime_type": content_type
                        })
                    except Exception as img_err:
                        logger.warning(f"提取Word图片失败: {img_err}")
                        continue
        except Exception as e:
            logger.warning(f"Word图片提取失败: {e}")
        return images

    def _extract_images_from_markdown(self, file_path: str) -> list:
        """从Markdown提取图片信息"""
        images = []
        try:
            ...
            import re
            # 匹配 ![alt](url) 或 ![alt](path) 格式
            img_pattern = r'!\[([^\]]*)\]\(([^\)]+)\)'
            matches = re.findall(img_pattern, content)

            for index, (alt, path) in enumerate(matches, 1):
                images.append({
                    "type": "markdown_image",
                    "index": index,
                    "alt": alt,
                    "path": path,
                    "description": f"Markdown第{index}张图片: {alt}" if alt else f"Markdown第{index}张图片",
                    "note": f"图片路径/URL: {path}"
                })
        except Exception as e:
            logger.warning(f"Markdown图片提取失败: {e}")
        return images
